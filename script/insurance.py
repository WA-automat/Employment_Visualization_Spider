...
if __name__ == '__main__':
    predict_num = 4
    new_data = {}
    df = pd.read_excel('../data/保险支出.xlsx', sheet_name='Sheet1', index_col=0, header=0)
    ...
    for index in name_list.to_list():
        ...
        new_items = [list(item) for item in zip(list(range(2001, 2026)), insurance_data)]
        new_data[index] = new_items

    # with open("../data/insurance.json", "w", encoding="utf-8") as f:
